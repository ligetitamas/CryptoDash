...
const Home = ({coins, filter, setFilter, limit, setLimit, sortBy, setSortBy, loading, error}) => {
    ...
    return (
        <div>
            ...
            {loading && <Spinner color='whote'></Spinner>}
            {error && <div className="error">{error}</div>}

            {!loading && !error && (
                <main className="grid">
                    {filteredCoins.length>0 && filteredCoins.map((coin) => (
                        <CoinCard key={coin.name} coin={coin}></CoinCard>
                    ))}
                    {filteredCoins.length==0 && <p>No matching coins</p>}
                </main>
            )}
        </div>
    );
